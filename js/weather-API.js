let requestURL = "https://api.data.gov.sg/v1/environment/4-day-weather-forecast";
...
function displayResult() {
    ...
    if (request.status == 200) {
        ...
        forecasts = item[0]['forecasts'];   // forecasts is a collection with 4 day forecasts
        let date = forecasts[0]['date'];
        let forecast = forecasts[0]['forecast'];
        let temperature = forecasts[0]['temperature'];
        let result = "Weather forecast for " + date + ": " + forecast +
            " Temperature : " + temperature['low'] + "(low) to " + 
